...
public class Validar {
    ...
    public int validar(Persona per) {
        int r = 0; //Para contar el número de indexs que cumple una dondición
    ...
    public int guardar(Persona per) {
        String sql = "INSERT INTO registrocliente(Nombres, Correo) VALUES (?,?)";
        con = cn.Conexion();
        try {
            ps = con.prepareStatement(sql);
    ...
    public Persona mostrarPersona(int _id) {
        ...
        Persona p = null;
        try {
            String sql = "SELECT id, Nombres, Correo FROM registrocliente WHERE id=?";
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            ps.setInt(1, _id);
            rs = ps.executeQuery();
    ...
    public int eliminar(int _id) {
        String sql = "DELETE FROM registrocliente WHERE id=?";
        con = cn.Conexion();
        try {
            ps = con.prepareStatement(sql);
            ps.setInt(1, _id);
    ...
    public List<Persona> listarpersonas() {
        ...
        try {
            String sql = "SELECT id, Nombres, Correo FROM registrocliente";
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
